...
export async function middleware(request) {
    ...
    if(request.nextUrl.pathname.includes('/dashboard')) { //validates if the url includes 'dashboard' so we can implement logic from it
        if(jwt === undefined) {
            return NextResponse.redirect(new URL('/login', request.url))//make sure the token isnt undefined, if its, redirect it to login
        }
        try { //if token exists, lets make sure its valid
